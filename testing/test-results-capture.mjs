...
async function test() {
  ...
  console.log('\n=== COMPLETE DATA TABLE ===\n');
  console.log('Call | Persona | T | Fit | DialogueAct | Auth');
  console.log('-----|---------|---|-----|-------------|-----');
  let i = 1;
  results.forEach(r => {
    console.log(i.toString().padStart(4) + ' | ' + r.persona.padEnd(9) + ' | ' + r.turn + ' | ' + (r.fit || 'null').toString().padEnd(3) + ' | ' + (r.act || '?').padEnd(11) + ' | ' + (r.auth || '?'));
    i++;
  });

  console.log('\nTotal: ' + results.length + ' API calls');
}
